...
def test_executor_container_overrides(instance_cm: Callable[..., ContextManager[DagsterInstance]]):
    with instance_cm() as instance:
        recon_job = reconstructable(bar)
        loadable_target_origin = LoadableTargetOrigin(python_file=__file__, attribute="bar_repo")
        ...
        with in_process_test_workspace(
            ...
            # Test with ecs/container_overrides step tag
            container_overrides = {
            ...
            run_container_overrides = run_task_kwargs["overrides"]["containerOverrides"][0]

            assert run_container_overrides["name"] == "run"
            assert run_container_overrides["command"] == ["my-command"]
            assert run_container_overrides["resourceRequirements"] == [
                {"type": "GPU", "value": "1"}
            ]
